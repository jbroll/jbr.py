...
class header(object) :
    ...
    def writecards(self, cards, other):
    ...
    def write(self, other) :
        ...
        self.writecards("END" + " " * 77, other)
        self.writecards(" " * int((self.headbytes - (len(self.card)+1) * 80)), other)
...
class hdu(header) :
    def __init__(self, fp, primary=True, cards=None) :
        ...
        if hasattr(fp, 'read') :
            if self.databytes > 0 :
                self.data = np.fromfile(fp, dtype=bitpix2dtype[self.bitpix], count=self.datapixls)
